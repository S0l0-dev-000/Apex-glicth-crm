...
const Navbar = ({ user, onLogout }) => {
  ...
  const handlePasswordChange = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
              const res = await fetch('https://backend-2u3y37hm3-solos-projects-3bdcd80e.vercel.app/api/change-password', {
        method: 'POST',
  ...
  const handleEmailChange = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
              const res = await fetch('https://backend-2u3y37hm3-solos-projects-3bdcd80e.vercel.app/api/change-email', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
  ...
  const handleCreateAdmin = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
              const res = await fetch('https://backend-2u3y37hm3-solos-projects-3bdcd80e.vercel.app/api/create-admin', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
